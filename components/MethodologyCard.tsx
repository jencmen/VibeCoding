      ...
      <div className="space-y-4 mb-8 flex-grow">
        <div className="bg-gray-50 p-3 rounded-2xl border border-gray-100">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-bold text-gray-400 uppercase">מהירות</span>
            <span className={`text-xs font-black ${getScoreColor(data.speed)}`}>{data.speed}%</span>
          </div>
          <div className="h-1.5 w-full bg-white rounded-full overflow-hidden border border-gray-100">
            <div className={`h-full rounded-full ${getScoreBg(data.speed)} transition-all duration-1000`} style={{ width: `${data.speed}%` }}></div>
          </div>
        </div>
        
        <div className="bg-gray-50 p-3 rounded-2xl border border-gray-100">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-bold text-gray-400 uppercase">דיוק</span>
            <span className={`text-xs font-black ${getScoreColor(data.accuracy)}`}>{data.accuracy}%</span>
          </div>
          <div className="h-1.5 w-full bg-white rounded-full overflow-hidden border border-gray-100">
            <div className={`h-full rounded-full ${getScoreBg(data.accuracy)} transition-all duration-1000`} style={{ width: `${data.accuracy}%` }}></div>
          </div>
        </div>
      </div>
